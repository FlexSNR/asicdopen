...
func (lMgr *LagManager) CreateLag(ifName string, hashType int, ifIndexListStr string) (int32, error) {
	...
	lMgr.logger.Info("Lag has been created with lagId = ", lagId, " vlanId = ", vlanId)
	...
	lMgr.dbMutex.Unlock()
	//Add Lag Vlan to Vlan database
	if vlanId != -1 {
		_, _ = lMgr.vlanMgr.CreateLagVlan(&asicdServices.Vlan{
			AdminState:    INTF_STATE_UP,
			VlanId:        int32(vlanId),
			IntfList:      []string{""},
			UntagIntfList: []string{strconv.Itoa(int(lagIfIndex))},
		})
	}
...
func (lMgr *LagManager) DeleteLag(ifIndex int32) int {
	...
	//Remove vlan state used by deleted lag
	vlanId := lMgr.lagDB[ifIndex].vlanId
	_, _ = lMgr.vlanMgr.DeleteLagVlan(&asicdServices.Vlan{
		VlanId:        int32(vlanId),
		IntfList:      []string{""},
...
func (lMgr *LagManager) UpdateLag(ifIndex int32, hashType int, ifIndexListStr string) (int, error) {
	...
	lMgr.dbMutex.Unlock()
	lMgr.portMgr.SetPortConfigMode(pluginCommon.PORT_MODE_UNCONFIGURED, oldPortIfIndexList)
	lMgr.portMgr.SetPortConfigMode(pluginCommon.PORT_MODE_L2, ifIndexList)
	//Publish notification - ARPd listens to lag create notification
	msg := pluginCommon.LagNotifyMsg{
		IfIndex:     ifIndex,
