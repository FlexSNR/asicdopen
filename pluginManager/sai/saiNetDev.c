...
int SaiCreateLinuxIf(int port, char *ifName, sai_object_id_t *hostIfId)
{
    ...
    sai_hostif_attr[1].value.oid = port_list[port];
    ...
    if (rv != SAI_STATUS_SUCCESS) {
        syslog(LOG_ERR, "Failure mapping asic ports to linux interfaces: "
                "port, ifName - %d, %s", port, ifName);
    }

    /**
     * Using default switch MAC address to all ports MAC
     * Don't use ioctl to set MAC to interface
     */
    /*
...
int SaiDeleteLinuxIfForAsicPorts(int ifMapCount, ifMapInfo_t *ifMap)
{
    ...
    sai_status_t rv;
    knetIfInfo knetIfDBEntry;
    int localIfMapCount = (ifMapCount == ALL_INTERFACES)?
        MAX_NUM_PORTS: ifMapCount;
    ...
    for (i = 0; i < localIfMapCount; i++) {
        ...
        if ((ifMapCount == ALL_INTERFACES)
                || !strncmp(knetIfDB[dbIndex].ifName, ifMap[i].ifName,
                    NETIF_NAME_LEN_MAX))
        {
            rv = sai_hostif_api_tbl->remove_hostif(knetIfDB[dbIndex].ifId);
            if (rv != SAI_STATUS_SUCCESS) {
                syslog(LOG_ERR, "Failed to delete net interface mapping for "
                        "port, ifName - %d, %s.",
                        dbIndex, knetIfDB[dbIndex].ifName);
            }
            //Delete knet if info from database
            knetIfDB[dbIndex] = knetIfDBEntry;
        } else {
            syslog(LOG_ERR, "Invalid interface map arguments when attempting "
                    "to delete net interface : port, ifName - %d, %s",
                    ifMap[i].port, ifMap[i].ifName);
        }
    }
#endif
    return 0;
